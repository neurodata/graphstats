...
mouse.plot <- gs.plot.heatmap(gs.edge.log(mouse.dmri, edge.attr="weight", output.type="graph"), title="(C) Mouse dMRI Connectome", src.label="",
                              tgt.label="", edge.attr="weight", fill.name = "log(Weight)")
fly.plot <- gs.plot.heatmap(fly.mushroom.left, title="(B) Fly Mushroom Body Connectome", src.label="",
                            tgt.label="", degree=TRUE)
celegans.plot <- gs.plot.grid(list(celegans.herm.chem, celegans.herm.gap),
                              graph.names=c("chem", "gap"),
                              title="(A) C. Elegans Connectome", src.label="",
                              tgt.label="", edge.attr="weight", degree=TRUE)
human.plot <- gs.plot.heatmap_overlay(list(human.fmri, human.dmri), title="(D) Human MRI Connectome",
...
edge.dens$label <- factor(edge.dens$label, levels=conn.order)
...
mouse.best=c(4, 2)
mouse.ase.plot <- plot.dim(gs.embed.ase(mouse.dmri, edge.attr="weight")$X, mouse.best, pt.color=V(mouse.dmri)$level1, pt.shape=V(mouse.dmri)$hemisphere, title="(C) Mouse dMRI", color.title="Structure", shape.title="Hemisphere")
...
class.dat <- data.frame(performance=c(.742, .485, .761, .706, .900, .614), chance=c(.603, .603, .483, .267, .300, .414))
class.dat <- t(class.dat)
colnames(class.dat) <- c("(A.i)", "(A.ii)", "(B)", "(C)", "(D.i)", "(D.ii)")

table.plot <- tableGrob(class.dat, theme=theme_bw())
```

```{r}
grid.arrange(celegans.chem.ase.plot, fly.ase.plot, human.dmri.ase.plot, celegans.gap.ase.plot, mouse.ase.plot,
          human.fmri.ase.plot, nrow=2)
```
