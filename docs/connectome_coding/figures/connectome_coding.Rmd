...
```{r}
mouse.plot <- gs.plot.heatmap(mouse.duke, title="(A) Mouse dMRI Connectome", src.label="",
...
celegans.plot <- gs.plot.grid(celegans.male, title="(C) Celegans Connectome", src.label="",
...
```{r}
human.sbm <- gs.sbm.fit(human.mri, community.attribute = "lobe", edge.attrs=NULL)
human.sbm <- delete_vertices(human.sbm, "non-labelled")
gs.plot.sbm(human.sbm, edge.attr="dMRI")
```
