...
human.dmri.ase <- gs.embed.ase(human.mri, k=gorder(human.mri)-1, edge.attr="dMRI")
human.fmri.ase <- gs.embed.ase(human.mri, k=gorder(human.mri)-1, edge.attr="fMRI")
mouse.dim <- gs.dim.select(mouse.duke, edge.attr="weight", plot=TRUE)
mouse.ase <- gs.embed.ase(mouse.duke, k=gorder(mouse.duke)-1, edge.attr="weight")
fly.dim <- gs.dim.select(fly.left, edge.attr=NULL, plot=TRUE)
fly.ase <- gs.embed.ase(fly.left, k=gorder(fly.left)-1)
...
celegans.chem.ase <- gs.embed.ase(celegans.male, k=gorder(celegans.male)-1, edge.attr="chem")
celegans.gap.ase <- gs.embed.ase(celegans.male, k=gorder(celegans.male)-1, edge.attr="gap")
```
