...
plot4 <- ggplot(results4, aes(x=modal, y=pval, shape=modal, color=dataset, group=dataset)) +
  ...
  ylab("") +
  scale_y_continuous(trans=log10_trans(), limits=c(.001, 1), breaks=c(.001, .001, .01, .1, 1)) +
  labs(shape="Modality", color="Site") +
  theme_bw()
```
...
dwi.pair <- gs.plot.plot_matrix(dwi_pair_results$D, vfactor = TRUE, xlabel="Dataset", ylabel="Dataset",
                    vlist = dwi_pair_results$dsets, legend.name=TeX("$log_{10}(p)$"),
                    title = "DWI Paired Batch") +
  scale_fill_gradient(name=TeX("$p$-value"), trans="log", breaks=c(0.001, 0.01, 0.1, 1),
                      limits=c(0.001, 1), low="red", high="white")
```
### fMRI

```{r, fig.height=5, fig.width=6}
fmri.pair <- gs.plot.plot_matrix(fmri_pair_results$D, vfactor = TRUE,
                                 xlabel="Dataset", ylabel="Dataset",
                    vlist = fmri_pair_results$dsets, legend.name=TeX("$log_{10}(p)$"),
                    title = "fMRI Paired Batch") +
  scale_fill_gradient(name=TeX("$p$-value"), trans="log", breaks=c(0.001, 0.01, 0.1, 1),
                      limits=c(0.001, 1), low="red", high="white")
```
