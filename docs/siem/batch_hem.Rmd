...
case.experiment.pairwise <- function(models, datasets.labs, modal, nrep=1000) {
  ...
  for (i in 1:(length(dsets) - 1)) {
    ...
    for (j in ((i+1):length(dsets))) {
      ...
      results <- rbind(results, data.frame(dset1=dsets[i], dset2=dsets[j], pval=get_results(perm.result), modal=modal, size=sum(ss1)))
      results <- rbind(results, data.frame(dset1=dsets[j], dset2=dsets[i], pval=get_results(perm.result), modal=modal, size=sum(ss1)))
