...
case2.perm <- function(models, Z, i=1, j=2, ...) {
  ...
  for (i in 1:length(Zset)) {
    sis <- which(Zset == Zset[i])
    sips <- which(Zset != Zset[i])
    alt <- mean(sapply(1:length(sis), function(k) {
      sapply(sis[sis != k], function(l) {
        p[sis[k]] - p[sis[l]] - (q[sis[k]] - q[sis[l]])
        })}))
    null <- sapply(1:length(sis), function(k) {
      sapply(sis[sis != k], function(l) {
        p[sis[k]] - p[sis[l]] - (q[sis[k]] - q[sis[l]])
        })})
    P[i] <- sum(null > alt)/length(null)
  }
  return(list(P=P))
}
