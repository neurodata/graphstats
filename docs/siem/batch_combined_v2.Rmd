...
title <- c("Exploratory", "(%s.%s) Between Study Batch Effect", "Sessions", "Individuals", "Sexes", "Sites",
  ...
  for (pp in 1:2) {
    ...
    if (pp == 1) {
      ...
      plot <- ggplot(results, aes(x=p, y=dataset, fill=dataset)) +
        ...
        xlab(TeX("Connection Strength")) +
        ylab(TeX("Study")) +
        theme_bw() +
        ggtitle(sprintf("(%s.%s) Study-Wise Distributions", letters[i], roman[1])) +
        labs(shape="Modality", color="Study") +
        theme_bw() +
        theme(legend.position="none", axis.text=element_text(size=11), axis.title=element_text(size=14),
