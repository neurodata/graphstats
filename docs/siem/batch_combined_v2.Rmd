...
title <- c("Exploratory", "(%s.%s) Prevalence of Batch\n        Effects Between Studies", "Sessions", "Subjects", "Sexs", "Sites",
  ...
  for (pp in 1:2) {
    ...
    if (pp == 1) {
      ...
      plot <- ggplot(results, aes(x=p, y=dataset, fill=dataset)) +
        ...
        ylab(TeX("Study")) +
        theme_bw() +
        ggtitle(sprintf("(%s.%s) Estimate of Model Fit", letters[i], roman[1])) +
